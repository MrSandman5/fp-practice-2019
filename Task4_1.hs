...
instance Applicative FunMonad where
    pure = return
--`<*>` apply packed function f to packed value x 
    f <*> x = 
        f >>= \f' ->
            x >>= \x' -> 
                return (f' x')
...
--`>>=` operation apply function f to value in context of x, which returns the value in the context, a function is created in which the value is taken from the context (x $ y), and the specified function (f . x $ y) is applied to it. Next, the function (fun . f . x $ y) is taken from the resulting monad and the argument y is applied to it.
